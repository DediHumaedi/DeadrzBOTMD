...
try {
  ...
  //let ytLink = `http://yt-downloader.akkun3704.repl.co/?url=${url}&filter=audioonly&quality=highestaudio&contenttype=audio/mpeg`
  //let aLink = await shortUrl(ytLink)
  let ytLink = await fetch(`https://api.sun3haxor.my.id/api/ytmp3?url=${url}&apikey=LLVOdtcN`)
  let aLink = await ytLink.json()
...
👁️ *Views:* ${viewH}
  ...
  if (durationS > 500) return conn.sendMessage(m.chat, { text: `*Download:* ${aLink.result.result}\n\n_Duration too long..._` }, { quoted: msg })
  conn.sendMessage(m.chat, { audio: { url: aLink.result.result }, mimetype: 'audio/mpeg' })
...
👁️ *Views:* ${viewH}
  ...
  await conn.sendButton(m.chat, whmodsdev, wm, botdate, [
    ...
    m.reply(`Lagu Error  ! \nIni aja Download : https://tinyurl.com/2ydpdkwu`)
  }
