...
let name = await conn.getName(who)
 ...
 const ini = await conn.sendContactArray(m.chat, [
    [`${nomorown}`, `${await conn.getName(nomorown+'@s.whatsapp.net')}`, `👑 Developer Bot `, `🚫 Don't call me 🥺`, `dedihaxor@gmail.com`, `🇮🇩 Indonesia`, `🚀 https://sun3haxor.my.id`, `Jangan Lupa Bersyukur`],
    [`${conn.user.jid.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `🔥 Bot WhatsApp 🐣`, `📵 Don't spam/call me 😢`, `dedideadrz@gmail.com`, `🇮🇩 Indonesia`, `🚀 https://sshaxor.my.id`, `🤖 Dont Forget To Donate!`]
  ], fkontak)
